...
def _main(args):
    os.environ['PREFIX'] = os.path.join(os.environ.get("CONDA_PREFIX"), 'Library')
    ...
    src = os.path.join(os.getcwd(), "cudatoolkit-dev-post-install.py" )
    dst = prefix_bin_dir_path
    copy_files(src, dst)
    with open(dst / "cudatoolkit-dev-extra-args.json", "w") as f:
